...
class Yapayzeka():
    ...
    def veritabanigetir(self):
        ...
        coll=DATABASE.find({"$and":[
            ...
            {"data": {"$gt": []}},
            {"target":{"$in": ["yasam","ekonomi","bilim-teknoloji","kultur-sanat","saglik"]}}
        ]}, {"_id": 0, "url": 0})
        print(type(coll))
        # coll = DATABASE.find({"target":{"$nin":["turkiye","dunya","gundem"]}}, {"_id": 0, "url": 0})
        # for i in coll:
        #     with open("text.txt", "w+") as tx:
        #         tx.writelines(str(i))
        #         tx.write("\n")
        return coll
    ...
    def save_model(self):
        ...
        pipeline = Pipeline([
            ...
            ('clf', GradientBoostingClassifier(verbose=1,n_estimators=200),)])

        predicted = pipeline.fit(X_train, y_train)
        pcdump = pickle.dump(predicted, open("LinearSVChashstem.sav", "wb"))
        return pcdump
